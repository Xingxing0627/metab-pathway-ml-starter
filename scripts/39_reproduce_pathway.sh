...
python scripts/33_beam_expand.py --candidates data/external/candidates_demo_60.csv --out data/processed/pathway_tree.json --start_smiles "CC(=O)O" --k0 12 --k_decay 1 --max_depth 4 --no_loop
python scripts/34_render_path.py --tree data/processed/pathway_tree.json --candidates data/external/candidates_demo_60.csv --md_out reports/pathway_best.md --csv_out reports/pathway_best.csv
...
MD5_CANDS="$(md5sum data/external/candidates_demo_60.csv | awk '{print $1}')"
...
inputs:
  ...
  candidates_csv: data/external/candidates_demo_60.csv
  peaks_csv: $PEAKS
input_md5:
  events: $MD5_EVENTS
